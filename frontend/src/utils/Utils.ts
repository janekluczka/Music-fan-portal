...
import { isSpeakersVariantOutputDTOArray, SpeakersVariantOutputDTO } from "../store/api/result/dto/product/variant/SpeakersVariantOutputDTO";
...
function useHeadphonesVariantGroup(product: ProductOutputDTO): VariantSelectorResults {
    const [nominalImpedance, setNominalImpedance] = useState<string>("0");
    ...
    if (isHeadphonesVariantOutputDTOArray(product.variants)) {
        ...
        const impedancesVariantGroup: VariantsGroup = {
            name: "Impedance",
            isDisabled: (selectedImpedance: string) => !isHeadphonesVariantOutputDTOArray(product.variants) || 
                !product.variants.some(variant => variant.nominalImpedance + "" === selectedImpedance && variant.loudness + "" === loudness && variant.color === color),
            options: allImpedances,
            selectedOption: nominalImpedance,
            setSelectedOption: setNominalImpedance,
        };
        
        const loudnessVariantGroup: VariantsGroup = {
            name: "Loudness",
            isDisabled: (selectedLoudness: string) => !isHeadphonesVariantOutputDTOArray(product.variants) || 
            !product.variants.some(variant => variant.nominalImpedance + "" === nominalImpedance && variant.loudness + "" === selectedLoudness && variant.color === color),
            options: allLoudness,
            selectedOption: loudness,
            setSelectedOption: setLoudness,
        };

        const colorVariantGroup: VariantsGroup = {
            name: "Color",
            isDisabled: (selectedColor: string) => !isHeadphonesVariantOutputDTOArray(product.variants) || 
            !product.variants.some(variant => variant.nominalImpedance + "" === nominalImpedance && variant.loudness + "" === loudness && variant.color === selectedColor),
            options: allColors,
...
function useSpeakersVariantGroup(product: ProductOutputDTO): VariantSelectorResults {
    ...
    if (isSpeakersVariantOutputDTOArray(product.variants)) {
        ...
        const loudnessVariantsGroup: VariantsGroup = {
            name: "Loudness",
            isDisabled: (selectedLoudness: string) => !isSpeakersVariantOutputDTOArray(product.variants) || 
            !product.variants.some(variant => variant.loudness + "" === selectedLoudness && variant.color === color),
            options: allLoudness,
            selectedOption: loudness,
            setSelectedOption: setLoudness,
        };
        ...
        const colorsVariantsGroup: VariantsGroup = {
            name: "Color",
            isDisabled: (selectedColor: string) => !isSpeakersVariantOutputDTOArray(product.variants) || 
            !product.variants.some(variant => variant.loudness + "" === loudness && variant.color === selectedColor),
            options: allColors,
            selectedOption: color,
            setSelectedOption: setColor,
        };
